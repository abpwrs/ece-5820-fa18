...
describe OracleOfBacon do
  ...
  describe 'instance' do
    before(:each) do
      @orb = OracleOfBacon.new('fake_api_key')
    end
  ...
  describe 'parsing XML response' do
    ...
    describe 'for a normal match' do
      ...
      its(:data) do
        should == ['Carrie Fisher', 'Under the Rainbow (1981)',
  ...
  describe 'service connection' do
    ...
    it 'properly handles status 403 response (Unauthorized access) as OpenURI:HTTPError' do
      ...
      stub_request(:get, %r{http://oracleofbacon\.org}).to_raise(OpenURI::HTTPError.new('My 403 response', nil))
      @oob.find_connections
      expect(@oob.response.type).not_to be(:unlinkable)
      expect(@oob.response.type).not_to be(:graph)
      expect(@oob.response.type).not_to be(:spellcheck)
      expect(@oob.response.type).not_to be(:badinput)
      expect(@oob.response.type).to be(:unauthorized) # Replace with meaningful test
    ...
    it 'instantiates a Response object and assign to @response' do
      ...
      stub_request(:get, %r{http://oracleofbacon\.org}).to_return(body: body)
      @oob.find_connections
      expect(@oob.response.type).to eq(:graph)
      expect(@oob.response.data).to eq(['Carrie Fisher', 'Under the Rainbow (1981)', 'Chevy Chase', 'Doogal (2006)',
                                        'Ian McKellen'])
    end
  end
end
